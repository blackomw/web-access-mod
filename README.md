...
# 监控功能（v2.4新增）
curl -s "http://localhost:3456/monitor/enable?target=ID"                    # 启用监控
curl -s "http://localhost:3456/monitor/network?target=ID&filter=api"        # 获取网络请求
curl -s "http://localhost:3456/monitor/network/clear"                       # 清空网络请求
curl -s "http://localhost:3456/monitor/console?target=ID&filter=error"      # 获取控制台日志
curl -s "http://localhost:3456/monitor/console/clear"                       # 清空控制台日志
curl -s "http://localhost:3456/monitor/errors?target=ID"                    # 获取JS错误
curl -s "http://localhost:3456/monitor/errors/clear"                        # 清空JS错误
```

## browser-commands.mjs（bb-browser风格命令）

新增bb-browser风格的浏览器控制命令，提供更友好的CLI接口：

```bash
# 页面信息
node ~/.claude/skills/web-access/scripts/browser-commands.mjs get title    # 获取页面标题
node ~/.claude/skills/web-access/scripts/browser-commands.mjs get url      # 获取当前URL
node ~/.claude/skills/web-access/scripts/browser-commands.mjs get text "h1" # 获取元素文本(CSS选择器)

# 导航
node ~/.claude/skills/web-access/scripts/browser-commands.mjs back         # 后退
node ~/.claude/skills/web-access/scripts/browser-commands.mjs forward      # 前进
node ~/.claude/skills/web-access/scripts/browser-commands.mjs refresh      # 刷新

# 调试
node ~/.claude/skills/web-access/scripts/browser-commands.mjs network requests [filter]    # 查看网络请求
node ~/.claude/skills/web-access/scripts/browser-commands.mjs console [--clear]            # 查看/清空控制台
node ~/.claude/skills/web-access/scripts/browser-commands.mjs errors [--clear]             # 查看/清空JS错误
```
