...
const consoleMessages = []; // [{type, text, timestamp, url, lineNumber}]
const jsErrors = []; // [{message, url, lineNumber, columnNumber, stackTrace, timestamp}]
const monitoredTargets = new Set(); // 已启用监控的targetId
    ...
    const onMessage = (evt) => {
      ...
      if (msg.method === 'Network.loadingFailed') {
        ...
        if (existing) {
          ...
          existing.failureReason = errorText;
        }
      }
      ...
      if (msg.method === 'Runtime.consoleAPICalled') {
        ...
        consoleMessages.push({
          ...
          lineNumber: msg.params.stackTrace?.callFrames?.[0]?.lineNumber
        });
      ...
      if (msg.method === 'Runtime.exceptionThrown') {
        ...
        const error = {
          ...
          stackTrace: exceptionDetails.stackTrace?.callFrames?.map(f => 
            `${f.functionName || '<anonymous>'} (${f.url}:${f.lineNumber}:${f.columnNumber})`
          ).join('\n'),
          timestamp: Date.now()
        };
        ...
        // 限制存储数量
        if (jsErrors.length > 500) {
          jsErrors.splice(0, jsErrors.length - 500);
        }
      }
    };
...
// --- 启用监控 ---
async function enableMonitoring(targetId) {
  if (monitoredTargets.has(targetId)) return;
  
  const sessionId = await ensureSession(targetId);
  
  // 启用网络监控
  await sendCDP('Network.enable', {}, sessionId);
  
  // 启用Runtime监控（控制台和错误）
  await sendCDP('Runtime.enable', {}, sessionId);
  
  monitoredTargets.add(targetId);
  console.log(`[CDP Proxy] 已启用监控: ${targetId}`);
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // GET /monitor/enable?target=xxx - 启用监控
    if (pathname === '/monitor/enable') {
      await enableMonitoring(q.target);
      res.end(JSON.stringify({ enabled: true }));
    }

    // GET /monitor/network?target=xxx&filter=xxx - 获取网络请求
    else if (pathname === '/monitor/network') {
      await enableMonitoring(q.target);
      ...
      res.end(JSON.stringify({ 
        requests: filtered.slice(-100), // 最近100个
        total: filtered.length 
      }));
    }

    // GET /monitor/network/clear - 清空网络请求
    else if (pathname === '/monitor/network/clear') {
      networkRequests.clear();
      res.end(JSON.stringify({ cleared: true }));
    }

    // GET /monitor/console?target=xxx&filter=xxx - 获取控制台日志
    else if (pathname === '/monitor/console') {
      await enableMonitoring(q.target);
      const filter = q.filter;
      const filtered = filter ? consoleMessages.filter(m => m.text.includes(filter)) : consoleMessages;
      res.end(JSON.stringify({ 
        messages: filtered.slice(-100), // 最近100条
        total: filtered.length 
      }));
    }

    // GET /monitor/console/clear - 清空控制台日志
    else if (pathname === '/monitor/console/clear') {
      consoleMessages.length = 0;
      res.end(JSON.stringify({ cleared: true }));
    }

    // GET /monitor/errors?target=xxx&filter=xxx - 获取JS错误
    else if (pathname === '/monitor/errors') {
      await enableMonitoring(q.target);
      const filter = q.filter;
      const filtered = filter ? jsErrors.filter(e => e.message?.includes(filter)) : jsErrors;
      res.end(JSON.stringify({ 
        errors: filtered.slice(-50), // 最近50个
        total: filtered.length 
      }));
    }

    // GET /monitor/errors/clear - 清空JS错误
    else if (pathname === '/monitor/errors/clear') {
      jsErrors.length = 0;
      res.end(JSON.stringify({ cleared: true }));
    }
    ...
    // GET /info?target=xxx - 获取页面信息
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          ...
          '/monitor/enable?target=': 'GET - 启用监控',
          '/monitor/network?target=&filter=': 'GET - 获取网络请求',
          '/monitor/network/clear': 'GET - 清空网络请求',
          '/monitor/console?target=&filter=': 'GET - 获取控制台日志',
          '/monitor/console/clear': 'GET - 清空控制台日志',
          '/monitor/errors?target=&filter=': 'GET - 获取JS错误',
          '/monitor/errors/clear': 'GET - 清空JS错误',
        },
      }));
    }
  } catch (e) {
